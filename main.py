...
from spotted.spotted import Spotted, start_ui
...
if __name__ == '__main__':
  ...
  if FUNC == 'create_board':
    ...
    while frame_count > 0:
      ...
      corners, ids, rejectedImgPoints = cv.aruco.detectMarkers(frame, aruco_dictionary, parameters=parameters)
      cv.aruco.refineDetectedMarkers(frame, board, corners, ids, rejectedImgPoints)

      if ids is not None:
        charuco_ret, charuco_corners, charuco_ids = cv.aruco.interpolateCornersCharuco(corners, ids, frame, board)

        frame = cv.aruco.drawDetectedCornersCharuco(frame, charuco_corners, charuco_ids, (255))
        retval, rvec, tvec = cv.aruco.estimatePoseCharucoBoard(charuco_corners, charuco_ids, board, camera_matrix, distortion_coefficients, np.array([]), np.array([]))
    ...
    # print('camera_rvecs:', camera_rvecs)
    # print('camera_tvecs:', camera_tvecs)
    ...
    #   frame = cv.undistort(src=frame, cameraMatrix=camera_matrix, distCoeffs=distortion_coefficients0)

    #   corners, ids, rejectedImgPoints = cv.aruco.detectMarkers(frame, aruco_dictionary, parameters=parameters)


    #   for corner in corners:
    #     cv.cornerSubPix(frame, corner, winSize=(3, 3), zeroZone=(-1, -1), criteria=criteria)

    #   # frame = cv.aruco.drawDetectedMarkers(frame, corners, ids)

    #   # rvecs, tvecs = cv.aruco.estimate

    #   # for i in range(len(tvecs)):
    #   #   frame = cv.aruco.drawAxis(frame, camera_matrix, distortion_coefficients0, rvecs)

    #   ret, rvecs, tvecs = cv.aruco.estimatePoseCharucoBoard(corners, ids, board, camera_matrix, distortion_coefficients0, np.array([]), np.array([]))
